...
class LSTM_Model(object):
    def __init__(self, is_training=True):
        ...
        with tf.variable_scope("RNN") as self.RNN_name_scope:
            ...
            lstm_U = np.concatenate([ortho_weight(dim_proj),
                                     ortho_weight(dim_proj),
                                     ortho_weight(dim_proj)], axis=1)
            lstm_b = np.zeros((3 * 128,))

            lstm_W = tf.get_variable("lstm_W", shape=[dim_proj, dim_proj * 3],dtype=tf.float32,
                                          initializer=tf.constant_initializer(lstm_W))
            lstm_U = tf.get_variable("lstm_U", shape=[dim_proj, dim_proj * 3],dtype=tf.float32,
    ...
    def step(self, mask, input, cell_previous):
