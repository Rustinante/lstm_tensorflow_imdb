    ...
    def __init__(self, mode):
        ...
        if mode =='train':
            with tf.variable_scope("train"):
                ...
                self._inputs= tf.get_variable("inputs",initializer=self.train_features,shape=tf.shape(self.train_features))
        ...
        self.h = tf.zeros([BATCH_SIZE, dim_proj],dtype=np.float32)
        self.c = tf.zeros([BATCH_SIZE, dim_proj],dtype=np.float32)
        self.h_outputs = []
